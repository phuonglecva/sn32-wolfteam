...
SAMPLE_DATA_DIR = '/Users/nannan/Downloads/Bittensor_storage/test_sample_data'
...
def detect(text):
    ...
    apikey = 'c0e9686ba1f849950ce63fd12f0d4ae2fbbb4087'
    params = {
        'url': url,
        'apikey': apikey,
        'custom_headers': 'true',
...
def run_test():
    ...
    for f_name in file_names:
        ...
        if data is None:
            continue
        result = data['labels'][150:]
        pred = call_api(data['texts'][150:])
        if len(result) != len(pred):
            print(f"len(result) = {len(result)}, len(pred) = {len(pred)}")
            continue
        comp = [result[i] == pred[i] for i in range(len(result))]
        total_text += len(pred)
        ...
        print(
            f'file {f_name}, pred wrong: {comp.count(False)}, pred correct: {comp.count(True)}, accuracy: {comp.count(True) / len(pred)}, average: {total_pred_correct / total_text}, time_processing = {time_processing}ms')
